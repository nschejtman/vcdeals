...
class EMPEAScrapper @Inject()(fundDAO: FundDAO)(implicit ec: ExecutionContext) {
  ...
  def run() = {
    ...
    val tupledLinks: Array[(String, Url)] = empeaDoc.select("a[href]").toArray(Array.empty[Element]).filter(e => UrlValidator.isValid(e.attr("abs:href"))).map(e => (e.text(), Url(e.attr("abs:href"))))
